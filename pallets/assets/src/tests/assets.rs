...
mod set_metadata {
	...
	fn ok() {
		new_test_ext().execute_with(|| {
			...
			let name: Vec<u8> = "TestAsset".into();
			let symbol: Vec<u8> = "TASS".into();
	...
	fn must_be_signed() {
		new_test_ext().execute_with(|| {
			assert_ok!(Assets::create(Origin::signed(ALICE)));
			assert_noop!(
				Assets::set_metadata(Origin::none(), 0, "TestAsset".into(), "TASS".into()),
				BadOrigin
			);
		})
	}
	...
	fn must_exist() {
		new_test_ext().execute_with(|| {
			assert_noop!(
				Assets::set_metadata(Origin::signed(ALICE), 0, "TestAsset".into(), "TASS".into(),),
	...
	fn must_be_owner() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Assets::set_metadata(Origin::signed(BOB), 0, "TestAsset".into(), "TASS".into(),),
